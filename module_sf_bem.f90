...
!Intermediate rooms
       !WRITE(*,*) "step_1 BEM subroutine"        !_gl
       if (nlev.gt.2) then
	do ilev=2,nlev-1
...
!bottom room
	  ...
	  twal_int(6)=tflo(1,1)		
	   ...
	   !WRITE(*,*) "step_2 BEM subroutine"        !_gl				  	   
	   call int_rlrad(emins,emwin,sigma,twal_int,twin1D,&
     			  pwin,bw,bl,dzlev,rlint)
...
!top room
	  ...
	  twal_int(6)=trof(1)		
	   ...
	   !WRITE(*,*) "fino a int_rlrad"				
	   call int_rlrad(emins,emwin,sigma,twal_int,twin1D,&
     			  pwin,bw,bl,dzlev,rlint)
	  !WRITE(*,*) "dopo int_rlrad"
	  do ivw=1,6
...
!! Calculation of the sensible heat fluxes from the PV panels & electricity producti
     ...
     ! WRITE(*,*) "pv_frac_roof=", pv_frac_roof			!_gl
...
! ------------------------------------------------------------------
     ...
     !   WRITE(*,*) "nhourday", nhourday
     !    WRITE(*,*) "bw", bw
     !    WRITE(*,*) "bl", bl
     !    WRITE(*,*) "perflo", perflo
     !    WRITE(*,*) "hsesf", hsesf
     !    WRITE(*,*) "hsequip", hsequip, "dimensioni", SIZE(hsequip)
     !    WRITE(*,*) "hseqocc", hseqocc(ilev)
     !    WRITE(*,*) "hleqocc", hleqocc(ilev)
     !    WRITE(*,*) "ilev", ilev
     !    WRITE(*,*) "nlev", nlev
 	do ilev=1,nlev
...
!	
	  ...
	  shumlev(ilev)=shumlev(ilev)+(dt/(vollev*rhoint*latent))* &
        ...
        !WRITE(*,*) "fino a call cosump_total"

        call consump_total(nzcanm,nlev,consumpbuild,hsoutbuild, &
                           hsout,consump)

     ! WRITE(*,*) "end subroutine BEM"   !stampare variabili
     ! WRITE(*,*) "bl", bl
     ! WRITE(*,*) "bw", bw
     ! WRITE(*,*) "emwal", emwal
     ! WRITE(*,*) "tout", tout  
     ! WRITE(*,*) "press", press
     ! WRITE(*,*) "twal_int", twal_int    
     !WRITE(*,*) "rs", rs
     !WRITE(*,*) "rl", rl
     ! WRITE(*,*) "hswalout", hswalout
     !WRITE(*,*) "hswinout", hswalout
     !WRITE(*,*) "hsrof", hsrof
      return
      end subroutine BEM
!====6=8===============================================================72  
...
!
      ...
      real :: hf
       ...
       !WRITE(*,*) "start subroutine hsfluxpv"
...
!			Sensible heat flux
!			------------------
         !WRITE(*,*) "start subroutine fluxeqocc"
	 Af=bw*bl
